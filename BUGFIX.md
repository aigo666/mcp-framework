...
本次修复了Word和PDF文档处理工具的关键Bug，增强了工具的兼容性和稳定性。
...
## Bug 2: Word文档中嵌入外部文档被误识别为图片导致会话终止
...
## Bug 3: PDF文档中嵌入对象被误识别为图片导致会话终止

### 问题描述
与Word文档类似，PDF文档中的嵌入对象（表单、附件等）也可能被错误识别为图片，导致返回无法解析的图片格式，进而引起会话中断。

### 解决方案
1. 为PDF工具添加与Word工具相同的图片有效性验证机制
2. 优化图片处理流程，先验证再添加
3. 改进错误处理和用户提示信息

### 实现细节
- 添加相同的`_is_valid_image`方法到PDF工具中
- 改进图片处理流程：
  - 先收集所有有效图片，再集中添加到结果中
  - 只有当确认有有效图片时才添加图片相关信息
  - 更准确地区分嵌入对象和真实图片
- 更新用户提示，明确说明"嵌入对象不是有效图片"

## 测试验证
1. 成功解析Word 97-2003 (.doc)格式文件
2. 正确处理包含嵌入外部文档的Word文件，不再将嵌入文档误识别为图片
3. 正确处理包含嵌入对象的PDF文件，不再将非图片对象误识别为图片
4. 即使遇到异常情况，也能保持会话的正常进行

## 影响范围
- 增强了Word和PDF文档解析工具的兼容性和稳定性
- 提高了解析含有嵌入对象的文档的成功率
- 改善了错误提示信息的用户友好性 